...
- **Le logo** : `src/components/Mark.tsx`, le « toit-clé » copié de la maquette, tracés identiques. Icônes PNG et image de partage dans `public/`, engendrées depuis le même tracé SVG.
- **L'image de partage** (`public/og-image.jpg`, 1200 × 630) : JPEG **de base, avec en-tête JFIF, sans EXIF, non progressif**. Ces quatre mots comptent, parce qu'aucun encodeur ne les donne tous les quatre du premier coup : `sharp` n'écrit pas d'APP0, son `withMetadata()` ajoute de l'EXIF dont une orientation qui peut faire pivoter l'image, `mozjpeg` sort du progressif que plusieurs robots de prévisualisation refusent, et `sips` ajoute de l'EXIF aussi. On garde donc la sortie de base et on insère les 18 octets de l'APP0 juste après le marqueur de début. Jamais de WebP ici : plusieurs aperçus de réseaux sociaux ne savent pas le lire.
- **Les écrans** : `src/assets/screens/*.webp`, capturés sur `https://noecalmes.fr/maquette/bailora/` à ×3, **sans le châssis** : le téléphone est dessiné en CSS (`.phone` dans `index.css`, `src/components/PhoneFrame.tsx`), donc net sur tout écran. Pour les refaire, trois gestes à la capture :
  ...
  2. `width: 326px` sur `.blr-phone` (et 352 sur `.blr-phone-export`), plus `min-height: 705px` sur `.blr-content`, **plancher `tall` compris**. Ces deux nombres vont ensemble : voir la règle sur la silhouette, plus bas ;
...
- **Le hero montre le premier écran de l'app** (l'ouverture), la galerie montre les suivants, jamais deux fois le même. Les quatre partagent le même écran, **326 × 705**, ce qui aligne la rangée au pixel.
- **La silhouette est celle d'un iPhone, et ces deux nombres ne se changent pas séparément.** Un iPhone a un écran logique de 390 × 844, soit un rapport de 0,4621. La maquette rend le sien à 304 de large, ce qui est déjà plus allongé qu'un vrai téléphone ; en ramenant les écrans à une hauteur commune de 705 pour aligner la galerie, on tombait à 0,4318, et le téléphone paraissait étiré. Le remède n'est pas de raccourcir, ce qui couperait du contenu, mais d'**élargir** : à 326 de large, l'écran le plus haut (le tableau de bord) tient tout juste dans les 705 px qu'exige le rapport. Mesuré dans la page : **0,4624**, à 0,06 % près. La maquette est capturée à cette largeur, l'image n'est jamais déformée après coup. Changer la hauteur oblige donc à recalculer la largeur, et à recapturer.
